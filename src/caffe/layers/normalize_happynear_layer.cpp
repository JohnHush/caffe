...
#include "caffe/layers/normalize_happynear_layer.hpp"
...
void NormalizeHappyNearLayer<Dtype>::LayerSetUp(
  const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  normalize_type_ =
    this->layer_param_.normalize_happynear_param().normalize_type();
  fix_gradient_ =
    this->layer_param_.normalize_happynear_param().fix_gradient();
  bp_norm_ = this->layer_param_.normalize_happynear_param().bp_norm() && top.size() == 2;
}

template <typename Dtype>
void NormalizeHappyNearLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void NormalizeHappyNearLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void NormalizeHappyNearLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(NormalizeHappyNearLayer);
#endif

INSTANTIATE_CLASS(NormalizeHappyNearLayer);
REGISTER_LAYER_CLASS(NormalizeHappyNear);
